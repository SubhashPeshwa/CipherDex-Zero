...
export class MainScene extends Phaser.Scene {
  ...
  update(): void {
    ...
    if (!this.isDialogVisible && !this.isSettingsVisible) {
      if (!this.playerState.isMoving) {
        ...
        let newAnimation = '';
        let shouldUpdateOrientation = false;
        ...
        if (isMovementKeyPressed) {
          ...
          } else if (shouldUpdateOrientation) {
            // Even if we can't move, update the player's orientation
            const currentAnim = this.player.anims.currentAnim?.key || '';
            const idleAnimation = newAnimation.replace('walk-', 'idle-');
            if (currentAnim !== idleAnimation) {
